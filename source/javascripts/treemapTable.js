demo.directive('treemapTable', ['$rootScope', '$http', function($rootScope, $http) {
  return {
    ...
    require: '^babbage',
    scope: { showKey: '=?' },
    template: '<table class="treemap-table table table-condensed"><thead> <tr> <th class="key" ng-show="showKey" colspan="2" ng-click="sort(\'key\');" ng-class="{ sort: activeOrder(\'key\') }">{{keyTitle}} <div class="sort-container"><span class="caret"></span></div></th> <th ng-click="sort(\'name\')" ng-class="{ sort: activeOrder(\'name\'), asc: activeDirection(\'asc\'), desc: activeDirection(\'desc\')}">{{nameTitle}}<div class="sort-container"><span class="caret"></span></div></th> <th class="num betrag" ng-click="sort(\'value\')" ng-class="{ sort: activeOrder(\'value\'), asc: activeDirection(\'asc\'), desc: activeDirection(\'desc\')}">Betrag<div class="sort-container"><span class="caret"></span></div></th> <th class="num" ng-click="sort(\'value\')" ng-class="{ sort: activeOrder(\'value\'), asc: activeDirection(\'asc\'), desc: activeDirection(\'desc\')}">Anteil<div class="sort-container"><span class="caret"></span></div></th> </tr></thead><tbody> <tr ng-repeat="row in rows"> <td class="color"> <i ng-style="{color: row.color};" class="fa fa-square"></i></td><td class="key" ng-show="showKey"> {{row.key}} </td><td><a href ng-click="setTile(row);"> {{row.name}} </a></td> <td class="num">{{row.value_fmt}}</td> <td class="num">{{row.percentage}}</td> </tr> </tbody><tfoot><tr> <th colspan="2"> Summe </th> <th ng-show="showKey"></th><th class="num">{{summary.value_fmt}}</th> <th class="num">100%</th> </tr></tfoot></table>',
